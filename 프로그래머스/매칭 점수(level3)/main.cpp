...
int solution(string word, vector<string> pages) {
    int answer = 0;
    unordered_map<string, tuple<vector<string>, int, int, int, double>> page_infos;
    ...
    for(int i = 0; i < pages.size(); i++){
        ...
        bool is_closed = true;
        bool is_tag_init = false;
        string url = "";
        string external_url = "";
        string tag = "";
        string s;
        stringstream stream(pages[i]);
        
        while(stream >> s)
        {
            if(tag == "<meta"){
                if(s.find("https://") != string::npos){
                    for(int j = s.find("https://"); j < s.size(); j++){
                        if(s[j] == '"'){
                            break;
                        }
                        url += s[j];
            ...
            if(tag == "<a"){
                if(s.find("https://") != string::npos){
                    for(int j = s.find("https://"); j < s.size(); j++){
                        if(s[j] == '"'){
                            external_urls.push_back(external_url);
                            external_url = "";
                            break;
                        }
                        external_url += s[j];
        ...
        page_infos[url] = {external_urls, i + 1, words[word], external_urls.size(), words[word]};
    }
    
    for(auto iter = page_infos.begin(); iter != page_infos.end(); iter++){
        tuple<vector<string>, int, int, int, double> page_info = iter->second;
        vector<string> external_urls = get<0>(page_info);
        for(int i = 0; i < external_urls.size(); i++){
            get<4>(page_infos[external_urls[i]]) += (double)get<2>(page_info) / (double)get<3>(page_info);
        }
    }
    int max = -1;
    int idx = 0;
    for(auto iter = page_infos.begin(); iter != page_infos.end(); iter++){
        ...
        if(get<4>(page_info) >= max and get<1>(page_info) != 0){
            if(get<4>(page_info) == max and idx < get<1>(page_info) - 1){
                continue;
            }
            max = get<4>(page_info);
            answer = get<1>(page_info) - 1;
            idx = answer;
        }
    }
