...
unordered_map<string, pair<string, int>> revenue;

void divide_revenue(string name, int cost){
    if(name == "-")
        return;
    if(cost / 10 == 0)
        revenue[name] = {revenue[name].first, revenue[name].second + cost};
    else{
        revenue[name] = {revenue[name].first, revenue[name].second + cost - (cost / 10)};
        divide_revenue(revenue[name].first, cost / 10);
    }
}
...
vector<int> solution(vector<string> enroll, vector<string> referral, vector<string> seller, vector<int> amount) {
    ...
    for(int i = 0; i < enroll.size(); i++){
        revenue[enroll[i]] = {referral[i], 0};
    }

    for(int i = 0; i < seller.size(); i++){
        divide_revenue(seller[i], amount[i] * 100);
    }

    for(auto iter = enroll.begin(); iter != enroll.end(); iter++){
        answer.push_back(revenue[*iter].second);
    }
