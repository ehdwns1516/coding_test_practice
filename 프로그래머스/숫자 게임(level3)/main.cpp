...
int solution(vector<int> A, vector<int> B) {
    int answer = 0;
    priority_queue<int, vector<int>, less<int>> A_pq(A.begin(), A.end());
    priority_queue<int, vector<int>, less<int>> B_pq(B.begin(), B.end());
    
    while(!A_pq.empty() and !B_pq.empty()){
        if(A_pq.top() < B_pq.top()){
            A_pq.pop();
            B_pq.pop();
            answer++;
        }
