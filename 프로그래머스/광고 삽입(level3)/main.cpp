...
int timeToSec(string time){
    int result = stoi(time.substr(0,2)) * 60 * 60;
...
string solution(string play_time, string adv_time, vector<string> logs) {
    ...
    vector<int> time_line(play_sec, 0);
    long long max = 0;
    long long accum = 0;
    int cursor = 0;
    int result = 0;
    ...
    for(string log : logs){
        int start = timeToSec(log.substr(0,8));
        int finish = timeToSec(log.substr(9,8));
    ...
    while(cursor < play_sec){
        accum -= time_line[cursor - adv_sec];
        accum += time_line[cursor];
        if(max < accum){
            max = accum;
            result = cursor - adv_sec;
        }
        cursor++;
    }
