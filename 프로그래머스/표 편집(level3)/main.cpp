...
string solution(int n, int k, vector<string> cmd) {
    ...
    list<string> list_answer(n, "O");
    int before = 0;
    auto iter = list_answer.begin();
    while(!deleted.empty()){
        if(deleted.back() - before < 0){
            for(int i = 0; i < abs(deleted.back() - before); i++){
                iter--;
            }
        }
        else{
            for(int i = 0; i < deleted.back() - before; i++){
                iter++;
            }
        }
        list_answer.insert(iter, "X");
        iter--;
        before = deleted.back();
        deleted.pop_back();
    }
    
    for(auto it : list_answer){
        answer += it;
    }
    return answer;
}
