...
// first -> who, second -> count 리턴
pair<int, int> getWinWay(vector<vector<int>> board, vector<int> aloc, vector<int> bloc, int count, int turn){
    vector<pair<int, int>> future_result;
    if(!turn){
        if(!board[aloc[0]][aloc[1]]){
            return {1, count};
        }
        ...
        for(int i = 0; i < 4; i++){
            ...
            if(!board[x][y])
                continue;
            future_result.push_back(getWinWay(board, {x, y}, bloc, count + 1, 1));
        }
        if(!future_result.size()){
            return {1, count};
        }
        sort(future_result.begin(), future_result.end());
        
        if(!future_result[0].first)
            return {0, future_result[0].second};
        else
            return {1, future_result.back().second};
        
    }
    else{
        if(!board[bloc[0]][bloc[1]]){
            return {0, count};
        }
        ...
        for(int i = 0; i < 4; i++){
            ...
            if(!board[x][y])
                continue;
            future_result.push_back(getWinWay(board, aloc, {x, y}, count + 1, 0));
        }
        if(!future_result.size()){
            return {0, count};
        }
        ...
        for(int i = 0; i < future_result.size(); i++){
            if(future_result[i].first)
                return {1, future_result[i].second};
            if(!future_result[i].first and max < future_result[i].second)
                max = future_result[i].second;
        }
        return {0, max};
    }
}
...
int solution(vector<vector<int>> board, vector<int> aloc, vector<int> bloc) {
    pair<int, int> result = getWinWay(board, aloc, bloc, 0, 0);
