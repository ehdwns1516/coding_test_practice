/*
 https://programmers.co.kr/learn/courses/30/lessons/64063#qna
 호텔 방 배정
 문제 설명
...
unordered_map<long long, long long> empty_room;

long long findRoom(long long number){
    if(!empty_room[number]){
        empty_room[number] = number;
        return number;
    }
    else{
        long long after_room = findRoom(empty_room[number] + 1);
        empty_room[number] = after_room;
        return after_room;
    }
