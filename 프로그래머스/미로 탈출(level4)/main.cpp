/*
 https://programmers.co.kr/learn/courses/30/lessons/81304#qna
 미로 탈출
 문제 설명
...
unordered_map<int, int> is_trap;
bool visited[1000][1 << 10];

bool is_reverse(int curr_trap, int curr, int next){
    bool flag = false;
    if(is_trap[curr]){
        if(is_trap[next]){
            int reverse_count = 0;
            if((curr_trap & (1 << (is_trap[curr] - 1))))
                reverse_count++;
            if((curr_trap & (1 << (is_trap[next] - 1))))
                reverse_count++;
            if(reverse_count == 1)
                flag = true;
        }
...
int solution(int n, int start, int end, vector<vector<int>> roads, vector<int> traps) {
    ...
    for(int i = 0; i < traps.size(); i++){
        is_trap[traps[i] - 1] = i + 1;
    }
    
    for(int i = 0; i < roads.size(); i++){
        if(!edges[roads[i][0] - 1][roads[i][1] - 1] or edges[roads[i][0] - 1][roads[i][1] - 1] > roads[i][2]){
            edges[roads[i][0] - 1][roads[i][1] - 1] = roads[i][2];
        }
    }
    
    pq.push({0, start - 1, 0});
    while(!pq.empty()){
        auto [cost, curr, trap] = pq.top();
        pq.pop();
        if(curr == end - 1)
            return cost;
        if(visited[curr][trap])
            continue;
        
        visited[curr][trap] = true;
        trap = is_trap[curr] ? trap ^ 1 << (is_trap[curr] - 1) : trap;
        
        for(int i = 0 ; i < n; i++){
            if(i == curr)
                continue;
            bool check = is_reverse(trap, curr, i);
            if(check){
                if(!edges[i][curr])
                    continue;
                if(visited[i][trap])
                    continue;
                pq.push({cost + edges[i][curr], i, trap});
            }
            else{
                if(!edges[curr][i])
                    continue;
                if(visited[i][trap])
                    continue;
                pq.push({cost + edges[curr][i], i, trap});
            }
        }
    }
}
